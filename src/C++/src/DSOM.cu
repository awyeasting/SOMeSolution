...
void DSOM::trainOneEpochOneGPU(int gpu) {
...
void DSOM::trainOneEpochMultiGPU() {
	#pragma omp parallel
	{
...
void DSOM::updateGPUCodebook(int gpu) {
	gpuErrchk(cudaSetDevice(this->_gpus[gpu])); // Map internal gpu id to device id
